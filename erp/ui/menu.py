...
<style>
    ...
    /* Style pour les boutons sélectionnés - icône et texte en couleur d'accent */
    button.q-btn.w-full.text-left.selected-menu-item .q-icon,
    button.q-btn.w-full.text-left.selected-menu-item {
...
def create_styled_menu_button(
    ...
    if selected:
        btn.classes('selected-menu-item')
        btn.style(f'''
    ...
    btn.on('mouseenter', lambda: btn.style(
        f'background-color: {hex_to_rgba(theme.accent_color, 0.1)} !important;'
    ) if not selected else None)
    
    btn.on('mouseleave', lambda: (
        btn.style(
            f'background-color: {hex_to_rgba(theme.accent_color, 0.1)} !important; color: {theme.accent_color} !important;'
    ...
    with menu_container:
        ...
        def on_menu_item_click(label: str):
            ...
            if selected_item['value'] and selected_item['value'] in menu_buttons:
                ...
                old_btn.classes(remove='selected-menu-item')
                old_btn.style(f'''
                    background-color: transparent !important;
                    color: #2c3e50 !important;
                    border-left: 3px solid transparent !important;
            ...
            new_btn.classes(add='selected-menu-item')
            new_btn.style(f'''
                background-color: {hex_to_rgba(theme.accent_color, 0.1)} !important;
                border-left: 3px solid {theme.accent_color} !important;
                color: {theme.accent_color} !important;
                padding: 12px 16px 12px 0px !important;
                font-weight: normal !important;
